...
export const VideoCard = ({ show, isFocused, onReviewPress, onRatePress }: Props) => {
  ...
  // True fit-to-screen: fill container while maintaining 16:9 aspect ratio
  let videoWidth, videoHeight;
  
  // Calculate if we should fit by width or height
  const containerAspectRatio = width / height;
  const videoAspectRatio = ASPECT_RATIO; // 16:9
  ...
  if (!videoWidth || !videoHeight || videoWidth <= 0 || videoHeight <= 0) {
    // Invalid video dimensions detected, using fallback
    videoWidth = Math.min(width, height * ASPECT_RATIO);
  ...
  return (
    <View style={styles.container}>
      ...
      {showTitle && (
        <Animated.View style={[styles.fadingTitleBar, { opacity: fadeAnim }]}> 
        ...
        <View style={[
          ...
          {isLoading ? (
            ...
            <Pressable
              style={{ flex: 1 }}
              onPress={() => {
                // User tapped video to trigger playback
                // This user interaction can help trigger auto-play on mobile
              }}
            >
              <TrailerPlayer 
                key={show.id}
          ...
          >
            <LinearGradient
              ...
              style={[
                styles.gradient,
                isLandscape && styles.gradientLandscape
              ]}
              pointerEvents="box-none"
            >
              <View
                ...
                <Animated.View style={{ opacity: buttonOpacity }} pointerEvents="auto">
                  <Pressable
                    style={[
                      ...
                      isLandscape && styles.moreInfoButtonLandscape,
                      {
                        borderRadius: 8,
                  ...
                  >
                    <Text style={[
                      styles.moreInfoText,
                      isLandscape && styles.moreInfoTextLandscape
                    ]}>
                      More Info
                    </Text>
                  </Pressable>
          ...
          <View style={[
            styles.actionStack,
            isLandscape && styles.actionStackLandscape,
            {
              position: 'absolute',
              right: isLandscape ? 16 : 24,
              bottom: isLandscape ? 16 : 32,
              top: isLandscape ? '50%' : undefined,
              transform: isLandscape ? [{ translateY: -80 }] : [],
              zIndex: 60,
            ...
            <Pressable 
              style={[
                styles.actionButton, 
                isLandscape ? styles.actionButtonLandscape : styles.actionButtonPortrait
              ]}
              onPress={handleSavePress}
            >
              {isShowSaved(show.id)
                ? <BookmarkCheck size={isLandscape ? 20 : 24} color="#FF0050" fill="#FF0050" />
                : <Bookmark size={isLandscape ? 20 : 24} color="white" />}
            </Pressable>
            ...
            >
              <MessageSquare size={isLandscape ? 20 : 24} color="white" />
            </Pressable>
            ...
            >
              <Star size={isLandscape ? 20 : 24} color="white" />
            </Pressable>
            ...
            >
              <Share2 size={isLandscape ? 20 : 24} color="white" />
            </Pressable>
          </View>
        </View>
      </View>
      ...
      >
        <TouchableOpacity 
          style={[
            ...
            isLandscape && styles.modalBackdropLandscape
          ]} 
          activeOpacity={1} 
        ...
        >
          <View style={[
            styles.infoModal,
            isLandscape && styles.infoModalLandscape
          ]}>
            {/* Title and Platforms */}
            <View>
              ...
              <Text style={styles.infoPlatforms}>
                Available on: {Array.isArray((show as any)?.platforms) ? (show as any).platforms.join(', ') : 'Netflix, Prime Video, Hulu'}
              </Text>
            </View>

            {/* Synopsis and Close Button */}
            <View>
              <Text style={styles.infoSynopsis}>{show.synopsis}</Text>
      ...
      >
        <TouchableOpacity
          style={styles.shareOptionsBackdrop}
          activeOpacity={1}
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    position: 'relative',
    backgroundColor: '#000', // Black background to match app theme
  ...
  overlayLandscape: {
    left: 0,
    right: '35%', // Leave space for action buttons on the right
    bottom: 0,
  },
  gradientLandscape: {
    minHeight: 80,
  ...
  moreInfoButtonLandscape: {
    paddingHorizontal: 12,
    paddingVertical: 6,
    marginTop: 4,
  },
  moreInfoTextLandscape: {
    fontSize: 13,
  },
